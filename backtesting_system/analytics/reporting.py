...
def build_report(engine: BacktestEngine, risk_free_rate: float = 0.0) -> Dict[str, Any]:
    ...
    turnover_notional = sum(abs(t.entry_price * t.size) for t in engine.trades)
    turnover = turnover_notional / engine.initial_capital if engine.initial_capital else 0.0
    ...
    return {
        ...
        "sharpe": sharpe_ratio_annualized(returns, risk_free_rate=risk_free_rate),
        "sortino": sortino_ratio_annualized(returns, risk_free_rate=risk_free_rate),
        "cagr": annual_return,
...
def _json_default(obj: Any) -> Any:
    """JSON fallback for non-serializable scalars (numpy, complex, Path)."""
...
def write_report(report: Dict[str, Any], output_path: Path) -> None:
    ...
    with output_path.open("w", encoding="utf-8") as outfile:
        json.dump(report, outfile, indent=2, default=_json_default)
...
def write_trades_detailed(engine: BacktestEngine, output_path: Path, risk_free_rate: float = 0.0) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    with output_path.open("w", newline="", encoding="utf-8") as outfile:
        ...
        for idx, trade in enumerate(trades, start=1):
            ...
            running_sharpe = sharpe_ratio_annualized(running_returns, risk_free_rate=risk_free_rate)
