...
def main() -> None:
    ...
    for path in outputs:
        ...
        handler = DataHandler(data_source=data_source, validator=DataValidator())
    ...
    def run_strategy(strategy, label: str, partial_exits: bool = True):
        try:
            broker = SimulatedBroker(slippage_bps=0.0, fee_per_trade=0.0)
            ...
            backtest.run(
                ...
                end_date=datetime(2003, 12, 31, tzinfo=timezone.utc),
                show_progress=True,
            )
    ...
    buy_hold_engine, buy_hold_report = run_strategy(BuyHoldStrategy(params={}), "buy_hold")
    weekly_engine, weekly_report = run_strategy(WeeklyProfileStrategy(params={}), "weekly_profile")
    weekly_fixed_engine, weekly_fixed_report = run_strategy(
        WeeklyProfileStrategy(params={}),
        "weekly_profile_fixed_exit",
        partial_exits=False,
    )
    ict_engine, ict_report = run_strategy(ICTFramework(params={}), "ict_framework")
    price_action_engine, price_action_report = run_strategy(PriceActionStrategy(params={}), "price_action")
    composite_engine, composite_report = run_strategy(CompositeStrategy(params={}), "composite")
    summary_reports = {
    ...
    walk_forward = WalkForwardPipeline(
        ...
        strategy_factory=lambda: WeeklyProfileStrategy(params={}),
