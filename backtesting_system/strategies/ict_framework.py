...
from backtesting_system.utils.timezones import ASIA, LONDON, NY, is_in_ny_session, to_new_york
...
class KillzoneValidator:
    ...
    def is_valid_killzone(
        ...
        _ = timezone_offset
        ny_dt = to_new_york(dt)
        ny_hour = ny_dt.hour
        if ny_dt.weekday() == 0 and not allow_monday:
            return False
...
class PDAArrayDetector:
    def identify_fair_value_gaps(self, candles: List[Candle], pip_size: float = 0.0001) -> List[dict]:
        fvgs: List[dict] = []
        if len(candles) < 3:
            return fvgs
        for i in range(2, len(candles)):
            ...
            if c1.high < c3.low:
                fvgs.append({
                    ...
                    "size_pips": (c3.low - c1.high) / pip_size,
                    "index": i,
                })
            elif c1.low > c3.high:
                fvgs.append({
                    ...
                    "size_pips": (c1.low - c3.high) / pip_size,
                    "index": i,
                })
        return fvgs
    ...
    def validate_entry_at_pda(
        self,
        entry_price: float,
        arrays: dict,
        tolerance_pips: float = 8.0,
...
class CISDValidator:
    def detect_cisd(self, daily_candles: List[Candle], h1_candles: List[Candle]) -> dict:
        ...
        # Use only *closed* daily candles. The current day's daily candle
        # (``daily_candles[-1]``) is still being formed by the H1 bars
        # that follow it, so including it would leak information about
        # future price action into the signal. We work off
        # ``daily_candles[-2]`` as the "current" day and the prior
        # candle as the reference swing.
        prev_daily = daily_candles[-2]
        curr_daily = daily_candles[-1]
        swing_high_prev = daily_candles[-2].high
        swing_low_prev = daily_candles[-2].low
...
class OpeningRangeFramework:
    ...
    def calculate_static_opening_range(
        self,
        ...
        ordered = sorted(day_candles, key=lambda c: c.time)
        opening_candle = ordered[0]
        snapshot = [
            c
            for c in ordered
            if (to_new_york(c.time).hour, to_new_york(c.time).minute) <= (cutoff_hour, cutoff_minute)
        ]
        if not snapshot:
            snapshot = [opening_candle]

        day_low = min(c.low for c in snapshot)
        day_high = max(c.high for c in snapshot)
        framework = self.calculate_opening_range(opening_candle, day_low, day_high)
        framework["snapshot_cutoff"] = f"{cutoff_hour:02d}:{cutoff_minute:02d}"
        framework["snapshot_size"] = len(snapshot)
        return framework
...
class ICTFramework(Strategy):
    def __init__(self, params: dict):
        ...
        # Incremental daily-aggregation cache. Without this, calling
        # _daily_from_history on every bar re-runs aggregate_daily_by_ny
        # over the entire history - O(n^2) per backtest.
    ...
    def calculate_stop_loss(
        ...
        buffer = self.pips_to_price(buffer_pips, symbol)
        if direction == "long":
            obs = [
    ...
    def identify_ny_reversal(self, data) -> dict:
        ...
        if len(history) < 30:
            return {}
        day = to_new_york(history[-1].time).date()
        same_day = [c for c in history if to_new_york(c.time).date() == day]
        asia = [c for c in same_day if is_in_ny_session(c.time, ASIA)]
        london = [c for c in same_day if is_in_ny_session(c.time, LONDON)]
        ny = [c for c in same_day if is_in_ny_session(c.time, NY)]
    ...
    def generate_signals(self, data) -> dict:
        ...
        if day_candles:
            opening_range = self.opening_range.calculate_static_opening_range(
                day_candles,
        ...
        if not fvg_list:
            ...
            if direction == "long":
                stop = self.calculate_stop_loss("long", bar.close, h1_arrays, daily, symbol=symbol)
                target = self.project_target(bar.close, stop, "long")
                entry_ok, _source = self.pda_detector.validate_entry_at_pda(bar.close, h1_arrays, pip_size=self.get_pip_size(symbol))
                if not entry_ok:
                    return {}
            ...
            stop = self.calculate_stop_loss("short", bar.close, h1_arrays, daily, symbol=symbol)
